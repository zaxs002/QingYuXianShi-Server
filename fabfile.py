...
env.user = 'root'
env.sudo_user = 'root'
env.hosts = ['123.206.17.189']

db_user = 'root'
db_password = '1234'

_TAR_FILE = 'dist-zhitiao.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/zhitiao'
...
def deploy():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        sudo('chown nginx www')
        sudo('chown -R nginx %s' % newdir)
        sudo('chmod -R 760 %s/templates' % newdir)
        sudo('chmod -R 760 %s/static' % newdir)
    with settings(warn_only=True):
        sudo('supervisorctl stop zhitiao')
        sudo('supervisorctl start zhitiao')
